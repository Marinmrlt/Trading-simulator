...
import { EquitySnapshotPresenter } from '../presenters/equity-snapshot.presenter';
import { DepositDto } from '../dto/deposit.dto';
import { WithdrawDto } from '../dto/withdraw.dto';
import { GetEquityDto } from '../dto/get-equity-curve.dto';
...
export class WalletController {
    ...
    @ApiResponse({ status: 200, description: 'Portfolio summary with total USD value.', type: PortfolioPresenter })
    @Get('portfolio')
    public async getPortfolio(@Req() req): Promise<PortfolioPresenter> {
        const userId = req.user.id;
        const data = await this.walletService.getPortfolioSummary(userId);
        return new PortfolioPresenter(data);
    }

    @ApiOperation({ summary: 'Get equity curve', description: 'Historical portfolio value snapshots.' })
    @ApiResponse({ status: 200, description: 'Array of equity snapshots.', type: [EquitySnapshotPresenter] })
    @Get('equity-curve')
    public async getEquityCurve(@Req() req, @Query() query: GetEquityDto): Promise<EquitySnapshotPresenter[]> {
        const userId = req.user.id;
        const snapshots = await this.equitySnapshotService.getEquityCurve(userId, query.days);
        return snapshots.map(s => new EquitySnapshotPresenter(s));
    }
