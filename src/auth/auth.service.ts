...
export class AuthService {
    ...
    async getTokens(userId: string, email: string) {
        ...
        const at = await this.jwtService.signAsync(atPayload, { expiresIn: '15m', secret: process.env.JWT_SECRET || 'secretKey' });

        // Refresh Token
        const rtPayload = { sub: userId, email };
        const rt = await this.jwtService.signAsync(rtPayload, { expiresIn: '7d', secret: process.env.JWT_REFRESH_SECRET || 'refreshKey' });
